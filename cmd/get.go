...
func runGet(cmd *cobra.Command, args []string) error {
	...
	ptr, metadata, err := resolvePtr(ctx, ptrArg)
	if err != nil {
		return err
	}
	...
	app.Debugf("Fetching record with pointer: %s", ptr)
...
func resolvePtr(ctx context.Context, ptrArg string) (string, *source.SourceMetadata, error) {
	// Try to parse as a number (short reference)
	if num, err := strconv.Atoi(ptrArg); err == nil && num > 0 {
		return resolvePtrFromCache(ctx, num)
	}
	...
	if len(ptrArg) <= 12 && !strings.HasPrefix(ptrArg, "file://") && !strings.HasPrefix(ptrArg, "s3://") {
		return resolvePtrBySuffix(ctx, ptrArg)
	}
...
func resolvePtrFromCache(ctx context.Context, num int) (string, *source.SourceMetadata, error) {
	mgr, err := cases.NewManager()
	if err != nil {
		return "", nil, fmt.Errorf("failed to access pointer cache: %w", err)
	}

	cache, err := mgr.LoadPtrCache(ctx)
	if err != nil {
...
func resolvePtrBySuffix(ctx context.Context, suffix string) (string, *source.SourceMetadata, error) {
	mgr, err := cases.NewManager()
	if err != nil {
		return "", nil, fmt.Errorf("failed to access pointer cache: %w", err)
	}
