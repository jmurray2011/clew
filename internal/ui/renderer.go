...
// Uses stdout to maintain consistent ordering with other output.
func (r *Renderer) Status(format string, args ...any) {
	if r.quiet {
		return
	}
	msg := fmt.Sprintf(format, args...)
	_, _ = fmt.Fprintln(r.out, r.render(StatusStyle, msg))
}
...
func (r *Renderer) Warning(format string, args ...any) {
	msg := fmt.Sprintf(format, args...)
	_, _ = fmt.Fprintln(r.out, r.render(WarningStyle, "Warning: "+msg))
}
